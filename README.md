...
| **`bool()`**  | Lenient boolean. Only affirmative values are `true`; **everything else — including unrecognized values, empty strings, and unset vars — is `false`**. Never throws. | `"true"`, `"yes"` (case-insensitive, trimmed) become `true`. `"false"`, `"no"`, `""`, `"1"`, `"maybe"` become `false`. |
